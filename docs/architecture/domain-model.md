...
| **Work Package** | — | Project | Referenced by BoQ and Contract from outside, so it **must** be a root. Defined up front, independent lifecycle. |
...
| **BidStatus** | InDiscussion, Quoted, Shortlisted, Selected, Rejected, Withdrawn |
...
| status | BidStatus | `Selected` is what a Contract is created from; at most one Selected per work package. |
