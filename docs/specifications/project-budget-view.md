...
- **`ManualExchangeRateProvider` now supplies a cross EUR↔RON display rate** (it previously threw for cross-currency). Default `1 EUR = 5.24 RON`, overridable via `EXCHANGE_RATE_RON_PER_EUR`. No other code calls the port, so the behaviour change is self-contained.
