...
| [Authentication via Cloudflare Access](./specifications/cloudflare-access-authentication.md) | Approved | Cloudflare Access (Zero Trust) + Google login behind a Cloudflare Tunnel gates both the web UI and the MCP server to the four stakeholders via one email allow-list; retires the Entra External ID plan. API/Postgres stay private. |
